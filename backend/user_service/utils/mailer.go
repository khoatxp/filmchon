...
func (s *sendMail) SendResetPassword(ToUser string, FromAdmin string, Token string, Sendgridkey string, AppEnv string) (*EmailResponse, error) {
	h := hermes.Hermes{
		Product: hermes.Product{
			Name: "FilmChon",
			Link: "https://filmchon.com",
		},
	}
	var forgotUrl string
	if os.Getenv("APP_ENV") == "production" {
		forgotUrl = "https://filmchon.com/resetpassword/" + Token //this is the url of the frontend app
	} else {
	...
	email := hermes.Email{
		Body: hermes.Body{
			Name: ToUser,
			Intros: []string{
				"Welcome to FilmChon! Good to have you here.",
			},
	...
	from := mail.NewEmail("FilmChon", FromAdmin)
